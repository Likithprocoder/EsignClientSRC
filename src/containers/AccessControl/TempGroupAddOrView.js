                    ...
                    templateGroup.map((posts, index) => (
                        <div key={index} className='eachTempGroup'>
                            <div key={posts.code} className='groupNameCss'>
                                <span hidden={sessionStorage.getItem("roleID") === "1" ? false : true}>{posts.status === 0 ? <i class="fa fa-times" id="fafatimesid" style={{ fontSize: "25px", padding: "0px", color: "#f86c6b" }}></i> : <i className="fa fa-check" style={{ color: "green", fontSize: "25px", padding: "0px" }}></i>} &nbsp;</span>
                                <span>{posts.name}</span>
                            </div>
                            <div className='viewAndAddUserBtn' hidden={sessionStorage.getItem("roleID") === "1" ? false : true}>
                            ...
                            <div className='viewAndAddUserBtn' hidden={sessionStorage.getItem("roleID") === "1" ? false : true}>
                                {/* {console.log(posts)} */}
                                <button style={{ height: "fit-content" }} type='submit' onClick={() => getApiKeysList(posts.name, posts.code)} className='btn btn-success' disabled={posts.status === 0}>API Integrations</button>
                            </div>
                        </div>
                    ))
                }
